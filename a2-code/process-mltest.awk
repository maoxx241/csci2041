...
/^ *__check__/{
  ...
  fmt = "if not (%s) then raise (TestFail {loc=\"%s\"; msg=msg; code=\"%s\"}) else ();"
  $0 = sprintf(fmt,check_code,loc,test_code)
}


# Found a check_output() call, replace it with exception raise and
# contextural info on where the test contents. Look for a ref variable
# "msg" which will contain the specific message.
/^ *__check_output__/{
  debug_print("check found: " $0)
  $1=""                         # eliminate check() call
  ...
  # sub in values include the accumulated test_code variable which has
  # all the test code for the current test so far
  fmt = "if not (%s) then raise (TestFail {loc=\"%s\"; msg=(!msgref); code=\"%s\"}) else ();"
  $0 = sprintf(fmt,check_code,loc,test_code)
}
