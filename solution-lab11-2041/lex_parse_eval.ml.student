...
let rec eval_expr varmap expr =
  ...
  | Cond(c) -> begin                                       (* eval a condition *)
    raise (EvalError{msg="if/then/else not implemented"})  (* DELETE ME *)
(* DELETE THIS COMMENT START
      let test = Bool false (* EDIT ME *) in               (* evaluate the test in c.if_expr *)
      match test with                                      
